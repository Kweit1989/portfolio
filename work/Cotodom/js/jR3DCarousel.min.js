...
"ms "+a.animationCurve,opacity:0});f=g*d;h.css({transform:"translateZ("+-e+"px) rotateY("+-f+"deg)"});h.css({opacity:1})}function z(){d.find(".nav").css({backgroundColor:"rgba(201, 201, 235, 1)"}).eq(r%c).css({backgroundColor:"rgba(44, 62, 80, 1)"});u=Math.round(f/g);a.onSlideShow.call(this,h.find("."+a.slideClass).eq(u%c))}function C(){d.width("100%");h.hide();n=d.width()<a.width?d.width():a.width;t=n/L;d.css({width:n+"px",height:t+"px"});-1!=a.animation.indexOf("slide")?(e=n/2/Math.tan(Math.PI/
...
l.add(m).css({position:"absolute",top:"50%",zIndex:1,transform:"translateY(-50%)",fontSize:"4em",color:"rgba(201, 201, 235, 1)",cursor:"pointer",userSelect:"none"}).appendTo(d).hide();l.on("click",function(){r=u-1;y.run(a.animation,r);z()});m.on("click",function(){A()});d.on("mouseenter touchstart",function(){l.add(m).show()}).on("mouseleave touchcancel",function(){l.add(m).hide()});l.hover(function(){l.css({color:"rgba(201, 201, 235, 1)"})},function(){l.css({color:"rgba(201, 201, 235, 1)"})});
m.hover(function(){m.css({color:"rgba(201, 201, 235, 1)"})},function(){m.css({color:"rgba(201, 201, 235, 1)"})});b(document).on("keydown",function(c){var b=d[0].getBoundingClientRect();(b=0<b.bottom&&0<b.right&&b.left<(innerWidth||document.documentElement.clientWidth)&&b.top<(innerHeight||document.documentElement.clientHeight))&&37==c.which?(q(),"rtl"==a.rotationDirection?l.click():m.click()):b&&39==c.which&&(q(),"rtl"==a.rotationDirection?m.click():l.click())});M(d,function(b){q();"left"==
b?m.click():"right"==b?l.click():-1!=a.animation.indexOf("scroll")&&("down"==b?m.click():"up"==b&&l.click())})}function v(){for(var e=b("<div class=navigation />").css({position:"absolute",bottom:0,right:0}),g=0;g<c;g++)e.append("<div class=nav></div>");"circles"==a.navigation&&e.find(".nav").css({borderRadius:"12px"});e.find(".nav").css({display:"inline-block",margin:"5px",cursor:"pointer",backgroundColor:"rgba(201, 201, 235, 1)",width:"12px",height:"12px",transition:"all "+a.animationDuration+
"ms ease"}).first().css({backgroundColor:"rgba(44, 62, 80, 1)"});h.after(e);d.on("click",".nav",function(){B(b(this).index())})}(function(){-1!=a.animation.indexOf("slide")?(e=n/2/Math.tan(Math.PI/c),k=n/2*Math.tan(2*Math.PI/c)+"px"):-1!=a.animation.indexOf("scroll")?(e=t/2/Math.tan(Math.PI/c),k=t/2*Math.tan(2*Math.PI/c)+"px"):"fade"==a.animation&&(e=n/2/Math.tan(Math.PI/c),k=n/2*Math.tan(2*Math.PI/c)+"px");g=360/c;if(a.slides.length)for(var f=0;f<a.slides.length;f++){var l=b("<div class='jR3DCarouselSlide' data-index="+
